package com.blivtech.emptrack.ui.advance
...
import com.blivtech.emptrack.ui.advance.adapter.EmployeePickerAdapter
...
class AddEntryActivity : AppCompatActivity() {
    ...
    private fun updateTypeUI() {
        ...
        when (viewModel.entryType) {
            EntryType.OVERTIME -> binding.cardOvertime.setBackgroundResource(R.drawable.bg_card_working)
            EntryType.ADVANCE  -> binding.cardAdvance.setBackgroundResource(R.drawable.bg_card_weekoff)
            EntryType.BONUS    -> binding.cardBonus.setBackgroundResource(R.drawable.bg_card_holiday)
        }
        // header no longer recolored — bg_header (blue) set in XML
    }
        ...
        bonusTypes.forEach { type ->
            val chip = Chip(this).apply {
                ...
                setChipBackgroundColorResource(
                    if (type == viewModel.selectedBonusType) R.color.blue_light
                    else R.color.white
                )
                setOnClickListener {
    ...
    private fun showShiftPicker() {
        ...
        val adapter = com.blivtech.emptrack.ui.advance.adapter.ShiftPickerAdapter { shift ->
    ...
    private fun updateSaveButton() {
        ...
        binding.btnSave.backgroundTintList =
            android.content.res.ColorStateList.valueOf(
                if (isReady)  android.graphics.Color.parseColor("#2563EB")
